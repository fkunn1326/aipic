...
export default function App(_props) {
      ...
      (async () => {
        if (ctx.UserInfo.id !== undefined) {
          const { data, error } = await supabaseClient
            .from("follows")
            .select("*")
            .eq("following_uid", ctx.UserInfo.id);
          data?.map((id) => {
  ...
  const { data: followdata, error: followerror } = useSWR(
    follows !== undefined
      ? "../api/followimages/list" +
          access_limit +
          "&" +
          new URLSearchParams({ follows: follows }).toString()
      : null,
    fetcher
  );
    ...
    <div className="bg-white dark:bg-slate-900">
      ...
      <div className="mx-auto max-w-7xl py-8 px-4 sm:px-10">
        <div className="mt-6 w-full flex flex-row justify-between">
          ...
          <a className="text-sky-600 dark:text-sky-400">
            <Link href="/special/aipic_release">すべて見る</Link>
          </a>
        </div>
      </div>
      ...
      <div>
        <div className="mx-auto max-w-7xl py-8 px-4 sm:px-10">
          <div className="mt-6 w-full flex flex-row justify-between">
            ...
            <a className="text-sky-600 dark:text-sky-400">
              <Link href="/daily_ranking">すべて見る</Link>
            </a>
          </div>
        </div>
        <DailyRanking />
